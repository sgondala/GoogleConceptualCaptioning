        ...
        if len(self.coco_json) > 0:
            ...
            for root, dirs, files in os.walk(self.folder_path, topdown=False):
                for file in files:
                    ...
                    if isImage(fullpath):
                        self.files.append(fullpath)
                        image_id = fullpath.split('/')[-1][:-4] 
                        self.ids.append(image_id) # just order them sequentially
                        n = n + 1
    ...
    def get_batch(self, split, batch_size=None):
        ...
        for i in range(batch_size):
            ...
            try:
                ...
                print("Error in file ", self.files[ri])
                continue

            print("File name ", self.files[ri])
            print("image shape here", img.shape)
